...
type IntegerInputProps = ComponentProps<number> & {
  schema: ZodType<number>
  min?: number
  max?: number
}

export function IntegerInput({ value, onChange, schema, min, max }: IntegerInputProps) {
  const [raw, setRaw] = useState(value != null ? String(value) : '')

  const parsed = parseInt(raw, 10)
  const isEmptyOrIncomplete = raw === '' || raw === '-'
