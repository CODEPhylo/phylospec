...
public interface Bounded<T extends Comparable<T>> {
    ...
    default boolean withinBounds(T value) {
        boolean lowerCheck = lowerInclusive() ? value.compareTo(getLower()) >= 0 : value.compareTo(getLower()) > 0;

        boolean upperCheck = upperInclusive() ? value.compareTo(getUpper()) <= 0 : value.compareTo(getUpper()) < 0;
