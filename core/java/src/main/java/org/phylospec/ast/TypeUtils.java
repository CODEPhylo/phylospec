...
//        if (query.getParameterTypes().size() != ref.getParameterTypes().size()) return false;
//
//        for (int i = 0; i < query.getParameterTypes().size(); i++) {
//            boolean isPartiallyCovered = true;
//            for (ResolvedType queryTypeParam : query.getParameterTypes().get(i)) {
//                if (!isCoveredByTypeSet(queryTypeParam, ref.getParameterTypes().get(i), componentResolver)) {
...
//
//        for (int i = 0; i < query.getParameterTypes().size(); i++) {
//            assert query.getParameterTypes().get(i).size() == 1;
//            if (!partiallyCoversType(
//                    query.getParameterTypes().get(i).iterator().next(),
//                    ref.getParameterTypes().get(i),
//                    componentResolver
//            )) return false;
//        }
//
...
//        if (type.getParameterTypes().size() == 0) visitor.accept(type);
//
//        boolean fullyResolved = false;
//        for (int i = 0; i < type.getParameterTypes().size(); i++) {
//            Set<ResolvedType> parameterTypeSet = type.getParameterTypes().get(i);
//
