        ...
        return switch (phylospecTypeComponentName) {
            case "Tree" ->
                switch (propertyName) {
                    ...
                    case "nBranches" ->
                        new StringBuilder("2 * ").append(objectBuilder).append(".ntips() - 2");
                    case "nInternalNodes" ->
                        new StringBuilder().append(objectBuilder).append(".ntips() - 1");
            ...
            case "TaxonSet" ->
                switch (propertyName) {
                    ...
                    case "names" -> {
                        ...
                        RevStmt.Assignment tempTaxaStmt =
                                converter.addRevAssignment(new RevStmt.Assignment("temp_taxa", objectBuilder));
                        ...
                        RevStmt.Assignment tempNamesStmt = converter.addRevAssignment(new RevStmt.Assignment(
                                "temp_names",
                                new String[] {indexVarName},
                                new StringBuilder(tempTaxaName)
                                        .append("[")
                                        .append(indexVarName)
                                        .append("].getName()")));
