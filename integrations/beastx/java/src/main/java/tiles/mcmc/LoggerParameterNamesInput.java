...
public class LoggerParameterNamesInput extends TileInput<LoggerParameterNames, BeastXState> {

    private final String templateVariable;

    public LoggerParameterNamesInput(String templateVariable, boolean required) {
    ...
    public Set<Tile<?, BeastXState>> getCompatibleInputTiles(
        ...
        if (!(inputAstNode instanceof Expr.Array array)) {
            throw new FailedTilingAttempt.RejectedBoundary(
                    "BEAST X logger parameters must be an array."
            );
        }
        ...
        for (Expr element : array.elements) {
            ...
            throw new FailedTilingAttempt.RejectedBoundary(
                    "BEAST X logger parameters must be variable names or string loggable names."
            );
        }

        if (allPossibleInputTiles.isEmpty()) {
            LoggerParameterNamesTile tile =
                    new LoggerParameterNamesTile(names, List.of());
        ...
        Utils.visitCombinations(
                ...
                selectedInputTiles -> {
                    LoggerParameterNamesTile tile =
                            new LoggerParameterNamesTile(names, new ArrayList<>(selectedInputTiles));
    ...
    private static class LoggerParameterNamesTile extends Tile<LoggerParameterNames, BeastXState> {

        private final List<String> names;
        private final List<Tile<?, BeastXState>> inputTiles;

        private LoggerParameterNamesTile(
                List<String> names,
        ...
        @Override
        protected LoggerParameterNames applyTile(
            ...
            return new LoggerParameterNames(this.names);
        }
    }
}
